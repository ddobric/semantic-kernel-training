...
namespace AgentFramework_Samples.SqlAgent
{
    /// <summary>
    ...
    internal class SqlAgentSample
    {
        ...
        public static async Task RunAsync()
        {
            ...
            // Register SqlServerTool as a singleton so its tool methods can be resolved by the agent.
            ServiceCollection services = new();
            services.AddSingleton<SqlServerTool>();
            IServiceProvider serviceProvider = services.BuildServiceProvider();

            // Build the agent with three tools: GetCurrentUtcDate, GetScheme, QuerySqlTable.
            // The service provider is passed so the agent can resolve dependencies for tool invocations.
        ...
        ///   1. GetCurrentUtcDate — returns the current UTC timestamp for date-based queries.
        ///   2. GetScheme — returns the Markdown schema definition so the agent knows table/column names.
        ///   3. QuerySqlTable — executes a T-SQL query and returns pipe-delimited results (max 20 rows).
        /// The agent calls them in order: GetScheme → (optionally) GetCurrentUtcDate → QuerySqlTable.
        /// </summary>
        public class SqlServerTool
        {
            public SqlServerTool()
            {
                // Validate that the connection string is configured at startup.
            ...
            /// <summary>
            /// Tool 1: Returns the current UTC timestamp in ISO 8601 format.
            /// The agent calls this before composing date-filtered queries.
            /// </summary>
            ...
            /// <summary>
            /// Tool 3: Executes a T-SQL query against the configured SQL Server database.
            /// Results are returned as a pipe-delimited table, capped at 20 rows.
            /// The agent is instructed to call GetScheme first, then compose the query.
            /// </summary>
            ...
            /// <summary>
            /// Exposes all tool methods as <see cref="AITool"/> instances for registration with the agent.
            /// </summary>
